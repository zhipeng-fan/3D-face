...
uv_model = DoubleHeadUNet(3,3,32)
uv_model.to(device)
...
def train(base_model, uv_model, epoch):
    ...
    for i, data in loop:
        ...
        shape_map, color_map = uv_model(in_img)
...
for epoch in range(5, NUM_EPOCH):
    ...
    if isinstance(visualize_image, list):
        ...
        io.imsave("./result_uv/Epoch:{:02}_AllLoss:{:.6f}_ImgLoss:{:.6f}_LMKLoss:{:.6f}_RecogLoss:{:.6f}.png".format(epoch, all_loss, img_loss, lmk_loss, recog_loss), visualize_image)
    model2save = {'model': uv_model.state_dict(),
                  'optimizer': optimizer.state_dict()}
    torch.save(model2save, "./model_result_uv/epoch_{:02}_loss_{:.4f}_Img_loss_{:.4f}_LMK_loss{:.4f}_Recog_loss{:.4f}.pth".format(epoch+1, img_loss+LMK_LOSS_WEIGHT*lmk_loss, img_loss, lmk_loss, recog_loss))
